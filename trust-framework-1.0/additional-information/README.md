> [!CAUTION]
> This repository is a workspace copy for navigation, drafting, version control and collaboration. It is not the official statement of government policy and must not be relied on as such. For the official published policy, see the [UK digital verification services trust framework 1.0 pre-release on GOV.UK](https://www.gov.uk/government/publications/uk-digital-verification-services-trust-framework-1-0/uk-digital-verification-services-trust-framework-1-0-pre-release).

# Additional information

This folder is a placeholder for future repository material that is **not** part of the exact published UK digital verification services trust framework text, but that sits alongside version 1.0 of the framework — for example, editorial notes, errata, change logs, or cross-references added by maintainers of this repository.

Nothing in this folder should be treated as official Trust Framework policy.

At present the folder is intentionally empty other than a `.gitkeep` file.

## Back

- [Trust Framework 1.0 landing page](../README.md)
- [Repository home](../../README.md)
